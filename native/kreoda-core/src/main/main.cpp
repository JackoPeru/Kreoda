// kreoda-core — native sidecar entry (§7).
...
int RunSelfTest() {
  ...
  const std::string r1 = asText(kreoda::handle_command(ping));
  ...
  const std::string r2 = asText(kreoda::handle_command(mk));
  ...
  const std::string r3 = asText(kreoda::handle_command(box));
  ...
  if (asText(kreoda::handle_command(cyl)).find("\"status\":\"ok\"") ==
      std::string::npos) {
  ...
  if (asText(kreoda::handle_command(sph)).find("\"status\":\"ok\"") ==
      std::string::npos) {
...
#if KREODA_WITH_FLATBUFFERS
  const std::vector<uint8_t> r6 = kreoda::handle_command(mesh);
  {
    flatbuffers::Verifier verifier(r6.data(), r6.size());
    const auto* update =
        flatbuffers::GetRoot<Kreoda::Protocol::MeshUpdate>(r6.data());
...
#else
  const std::string r6 = asText(kreoda::handle_command(mesh));
  ...
  if (asText(kreoda::handle_command(bad)).find("\"status\":\"error\"") ==
      std::string::npos) {
  ...
  const std::string r8 = asText(kreoda::handle_command(setp));
#if KREODA_WITH_OCCT
...
#endif
#if KREODA_WITH_OCCT && KREODA_WITH_PLANEGCS
  ...
  const std::string r9 = asText(kreoda::handle_command(sk));
  ...
  const std::string r10 = asText(kreoda::handle_command(ex));
  ...
  std::printf("kreoda-core self-test OK\n");
  return 0;
}
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a == "--version") {
      std::printf("kreoda-core 0.1.0 protocol=1 occt=%s\n",
#if KREODA_WITH_OCCT
                  "8.0.1"
#else
                  "stub"
  ...
  kreoda::DocumentStore::instance().create("doc-bootstrap");
  ...
  kreoda::protocol::FrameDecoder decoder;
  ...
  while ((ch = std::cin.get()) != EOF) {
    ...
    try {
      ...
      decoder = kreoda::protocol::FrameDecoder();
      continue;
    }
    for (auto& payload : frames) {
      ...
      const int v = kreoda::json_int_field(req, "protocolVersion", -1);
      std::vector<uint8_t> resp;
      if (v != 1) {
        const std::string rid =
            kreoda::json_string_field(req, "requestId", "");
        resp = kreoda::make_response(
            rid, "error",
            "\"errorCode\":\"BAD_PROTOCOL\",\"errorMessage\":\"expected "
            "protocolVersion 1\"");
      } else {
        resp = kreoda::handle_command(req);
      }
      ...
      WriteAll(kreoda::protocol::frame(resp));
    }
  }
