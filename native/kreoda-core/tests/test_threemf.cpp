...
TEST(ThreeMF, ExportImportRoundTrip) {
  kreoda::DocumentStore::instance().create("threemf-test-doc");
  std::string err;
  ASSERT_TRUE(kreoda::CreateBoxFeature("threemf-box", 100, 60, 10, &err))
      << err;

  const fs::path file =
      fs::temp_directory_path() / "kreoda-threemf-roundtrip.3mf";
  std::error_code ec;
  fs::remove(file, ec);
...
TEST(ThreeMF, EmptyDocumentExportFailsHonestly) {
  kreoda::DocumentStore::instance().create("threemf-test-empty");
  std::string err;
  EXPECT_FALSE(kreoda::ExportThreeMF(
      "C:\\Windows\\Temp\\kreoda-empty.3mf", &err));
