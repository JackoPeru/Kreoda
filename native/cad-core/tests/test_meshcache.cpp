...
TEST(MeshCache, HitIsIdenticalAndCommitInvalidates) {
  ...
  ASSERT_TRUE(has(rpc("{\"protocolVersion\":1,\"requestId\":\"m2\","
                  ...
                  "\"status\":\"ok\""));
  const std::vector<uint8_t> mesh1 = intentcad::handle_command(
      "{\"protocolVersion\":1,\"requestId\":\"m3\","
      "\"documentId\":\"mcache-doc\",\"type\":12,"
      "\"featureId\":\"mcache-box\",\"lod\":1}");
  const auto* u1 = intentcad_test::meshRoot(mesh1);
  ...
  const std::vector<uint8_t> mesh2 = intentcad::handle_command(
      "{\"protocolVersion\":1,\"requestId\":\"m4\","
      "\"documentId\":\"mcache-doc\",\"type\":12,"
      "\"featureId\":\"mcache-box\",\"lod\":1}");
  const auto* u2 = intentcad_test::meshRoot(mesh2);
  ...
  ASSERT_TRUE(has(rpc("{\"protocolVersion\":1,\"requestId\":\"m5\","
                      ...
                      "\"valueMm\":200}"),
                  "\"status\":\"ok\""));
  const std::vector<uint8_t> mesh3 = intentcad::handle_command(
      "{\"protocolVersion\":1,\"requestId\":\"m6\","
      "\"documentId\":\"mcache-doc\",\"type\":12,"
      "\"featureId\":\"mcache-box\",\"lod\":1}");
  const auto* u3 = intentcad_test::meshRoot(mesh3);
