...
TEST(Step, ExportImportRoundTrip) {
  intentcad::DocumentStore::instance().create("step-test-doc");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("step-box", 100, 60, 10, &err))
      << err;

  const fs::path file =
      fs::temp_directory_path() / "intentcad-step-roundtrip.step";
  std::error_code ec;
  fs::remove(file, ec);
...
TEST(Step, EmptyDocumentExportFailsHonestly) {
  intentcad::DocumentStore::instance().create("step-test-empty");
  std::string err;
  EXPECT_FALSE(
      intentcad::ExportStep("C:\\Windows\\Temp\\intentcad-empty.step", &err));
