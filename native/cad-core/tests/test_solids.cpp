...
TEST(Solids, FuseAddsVolumesMinusOverlap) {
  NewDoc("bl1");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("ba", 100, 50, 20, &err)) << err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("bb", 40, 40, 40, &err)) << err;
  ASSERT_TRUE(intentcad::CreateBooleanFeature("bf", "fuse", "ba", "bb", &err))
      << err;
  // bb sits in the corner of ba: overlap 40x40x20 = 32000.
  EXPECT_NEAR(VolumeOf("bf"), 100000 + 64000 - 32000, 1.0);
  EXPECT_FALSE(intentcad::CreateBooleanFeature("bf2", "bogus", "ba", "bb", &err));
  EXPECT_FALSE(intentcad::CreateBooleanFeature("bf3", "fuse", "ba", "ba", &err));
  EXPECT_FALSE(intentcad::CreateBooleanFeature("ba", "fuse", "ba", "bb", &err));
}

TEST(Solids, CutSubtractsTool) {
  NewDoc("bl2");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("ca", 100, 50, 20, &err)) << err;
  ASSERT_TRUE(intentcad::CreateCylinderFeature("cb", 10, 60, &err)) << err;
  ASSERT_TRUE(intentcad::CreateBooleanFeature("cc", "cut", "ca", "cb", &err))
      << err;
  EXPECT_LT(VolumeOf("cc"), 100000.0);
  // Common keeps the overlap only.
  ASSERT_TRUE(intentcad::CreateBooleanFeature("cd", "common", "ca", "cb", &err))
      << err;
...
TEST(Solids, HoleThroughExactVolume) {
  NewDoc("hl1");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("ha", 100, 50, 20, &err)) << err;
  ASSERT_TRUE(intentcad::CreateHoleFeature("hh", "ha", "box.+Z", 50, 25, 8,
                                           "throughAll", 0, &err))
      << err;
  // Cylinder removed: pi * 16 * 20.
  EXPECT_NEAR(VolumeOf("hh"), 100000 - 3.14159265358979 * 16 * 20, 1.0);
  EXPECT_FALSE(intentcad::CreateHoleFeature("hh2", "ha", "box.+Z", 50, 25, -8,
                                            "throughAll", 0, &err));
  EXPECT_FALSE(intentcad::CreateHoleFeature("hh3", "ha", "nope", 50, 25, 8,
                                            "throughAll", 0, &err));
}
...
TEST(Solids, HoleBlindDepth) {
  ...
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("ba", 100, 50, 20, &err)) << err;
  ASSERT_TRUE(intentcad::CreateHoleFeature("hb", "ba", "box.+Z", 50, 25, 8,
                                           "blind", 10, &err))
      << err;
  ...
  ASSERT_TRUE(intentcad::CreateHoleFeature("hb2", "ba", "box.+Z", 10, 10, 8,
                                           "blind", 500, &err))
      << err;
}
...
TEST(Solids, HoleNeedsPlanarFace) {
  NewDoc("hl3");
  std::string err;
  ASSERT_TRUE(intentcad::CreateSphereFeature("sa", 30, &err)) << err;
  EXPECT_FALSE(intentcad::CreateHoleFeature("sh", "sa", "sph.all", 0, 0, 8,
                                            "throughAll", 0, &err));
}
...
TEST(Solids, HoleSurvivesTargetResize) {
  NewDoc("hl4");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("ta", 100, 50, 20, &err)) << err;
  ASSERT_TRUE(intentcad::CreateHoleFeature("th", "ta", "box.+Z", 50, 25, 8,
                                           "throughAll", 0, &err))
      << err;
  // Widen the target: the hole re-resolves on the same top face (§3–§4).
  ASSERT_TRUE(intentcad::RebuildFeature("ta", "widthMm", 150, &err)) << err;
...
TEST(Solids, HoleDiameterEdit) {
  NewDoc("hl5");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("ea", 100, 50, 20, &err)) << err;
  ASSERT_TRUE(intentcad::CreateHoleFeature("eh", "ea", "box.+Z", 50, 25, 8,
                                           "throughAll", 0, &err))
      << err;
  ASSERT_TRUE(intentcad::RebuildFeature("eh", "diameterMm", 10, &err)) << err;
...
TEST(Solids, FilletRoundsEdges) {
  NewDoc("fl1");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("fa", 100, 50, 20, &err)) << err;
  ASSERT_TRUE(intentcad::CreateFilletFeature(
  ...
  std::string bigErr;
  EXPECT_FALSE(
      intentcad::CreateFilletFeature("ff2", "fa",
  ...
  EXPECT_FALSE(intentcad::CreateFilletFeature("ff3", "fa", {"fa:edge.nope"}, 3,
                                              &err));
}
...
TEST(Solids, ChamferCutsCorners) {
  NewDoc("ch1");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("ka", 100, 50, 20, &err)) << err;
  ASSERT_TRUE(intentcad::CreateChamferFeature(
...
TEST(Solids, CutToEmptyAndDisjointCommonFailHonestly) {
  NewDoc("bl4");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("da", 100, 50, 20, &err)) << err;
  // Same geometry, different UUID: cut removes everything → honest error.
  ASSERT_TRUE(intentcad::CreateBoxFeature("db", 100, 50, 20, &err)) << err;
  EXPECT_FALSE(intentcad::CreateBooleanFeature("dc", "cut", "da", "db", &err));
  EXPECT_NE(err.find("empty"), std::string::npos) << err;
  // Disjoint common: nothing shared → honest error, no fake body.
  ASSERT_TRUE(intentcad::CreateBoxFeature("dd", 10, 10, 10, &err)) << err;
  ...
  ASSERT_TRUE(intentcad::CreateBooleanFeature("de", "common", "da", "dd", &err))
      << err;
...
TEST(Solids, HoleOnExtrudedFace) {
  ...
  ASSERT_TRUE(intentcad::CreateSketchFeature("sx", "XY", m, &err)) << err;
  ASSERT_TRUE(intentcad::CreateExtrudeFeature("ex", "sx", 20, &err)) << err;
  ...
  intentcad::ShapeRecord ex;
  ASSERT_TRUE(intentcad::ShapeStore::instance().get("ex", &ex));
  double origin[3], xa[3], ya[3], normal[3];
  ASSERT_TRUE(intentcad::FaceFrameInfo(ex.shape, "ex", "Extrude", "extrude.+Z",
                                       origin, xa, ya, normal));
  ...
  ASSERT_TRUE(intentcad::CreateHoleFeature("hx", "ex", "extrude.+Z", lx, ly, 8,
                                           "throughAll", 0, &err))
      << err;
...
TEST(Solids, ChamferRebuildAfterResize) {
  NewDoc("ch2");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("ra", 100, 50, 20, &err)) << err;
  ASSERT_TRUE(intentcad::CreateChamferFeature(
                  "rc", "ra", {"ra:edge.lin.box.+X~box.+Z"}, 3, &err))
      << err;
  const double before = VolumeOf("rc");
  ASSERT_TRUE(intentcad::RebuildFeature("ra", "widthMm", 150, &err)) << err;
...
TEST(Solids, SaveOpenKeepsHoleAndFillet) {
  NewDoc("sv1");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("pa", 100, 50, 20, &err)) << err;
  ASSERT_TRUE(intentcad::CreateHoleFeature("ph", "pa", "box.+Z", 50, 25, 8,
                                           "throughAll", 0, &err))
      << err;
  ASSERT_TRUE(intentcad::CreateFilletFeature(
  ...
  const fs::path icad = fs::temp_directory_path() / "intentcad-solids.icad";
  ...
  intentcad::ShapeRecord rh, rf;
  ASSERT_TRUE(intentcad::ShapeStore::instance().get("ph", &rh));
  ASSERT_TRUE(intentcad::ShapeStore::instance().get("pf", &rf));
