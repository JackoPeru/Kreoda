...
TEST(Stl, ExportImportRoundTrip) {
  intentcad::DocumentStore::instance().create("stl-test-doc");
  std::string err;
  ASSERT_TRUE(intentcad::CreateBoxFeature("stl-box", 100, 60, 10, &err))
      << err;

  const fs::path file =
      fs::temp_directory_path() / "intentcad-stl-roundtrip.stl";
  std::error_code ec;
  fs::remove(file, ec);
  ASSERT_TRUE(intentcad::ExportStl(file.string(), &err)) << err;
  ...
  ASSERT_TRUE(intentcad::ImportStl(file.string(), &ids, &err)) << err;
...
TEST(Stl, EmptyDocumentExportFailsHonestly) {
  intentcad::DocumentStore::instance().create("stl-test-empty");
  std::string err;
  EXPECT_FALSE(intentcad::ExportStl(
      "C:\\Windows\\Temp\\intentcad-empty.stl", &err));
