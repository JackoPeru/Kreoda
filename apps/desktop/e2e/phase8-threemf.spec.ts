// Phase 8 E2E (§61): 3MF mesh export/import round-trip through the UI.
// Box → export .3mf → import replaces the doc with MeshImport bodies at
...
const THREEMF = path.join(os.tmpdir(), "intentcad-phase8-e2e.3mf");
const ICAD = path.join(os.tmpdir(), "intentcad-phase8-3mf-e2e.icad");
...
test("3MF export → import → persist", async () => {
  ...
  try {
    ...
    // 2. Export 3MF (same Save path the toolbar uses — extension branches).
    await window.evaluate(
      ({ file }) =>
        ...
        ).__intentcad_test.saveIcad(file),
      { file: THREEMF },
    );
    ...
    const imported = (await window.evaluate(
      ({ file }) =>
        ...
        ).__intentcad_test.openIcad(file),
      { file: THREEMF },
    )) as Snapshot;
    ...
    await window.screenshot({ path: path.join(HERE, "phase8-threemf.png") });
  } finally {
    await app.close();
  }
});
