// Phase 8 E2E (§61): glTF mesh export/import round-trip through the UI.
// Box → export .gltf → import replaces the doc with MeshImport bodies
...
const GLTF = path.join(os.tmpdir(), "intentcad-phase8-e2e.gltf");
const ICAD = path.join(os.tmpdir(), "intentcad-phase8-gltf-e2e.icad");
...
test("glTF export → import → persist", async () => {
  ...
  try {
    ...
    // 2. Export glTF (same Save path the toolbar uses — extension branches).
    ...
    await window.screenshot({ path: path.join(HERE, "phase8-gltf.png") });
  } finally {
    await app.close();
  }
});
