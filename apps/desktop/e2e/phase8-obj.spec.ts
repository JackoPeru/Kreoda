// Phase 8 E2E (§61): OBJ mesh export/import round-trip through the UI.
// Box → export .obj → import replaces the doc with MeshImport bodies
...
const OBJ = path.join(os.tmpdir(), "intentcad-phase8-e2e.obj");
const ICAD = path.join(os.tmpdir(), "intentcad-phase8-obj-e2e.icad");
...
test("OBJ export → import → persist", async () => {
  ...
  try {
    ...
    // 2. Export OBJ (same Save path the toolbar uses — extension branches).
    ...
    await window.screenshot({ path: path.join(HERE, "phase8-obj.png") });
  } finally {
    await app.close();
  }
});
